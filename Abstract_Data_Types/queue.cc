...
class Queue {
  private:
    Vector<T> storage_;
  public:
    void enqueue(const T& element) { storage_.push_back(element); }
    void dequeue() { assert(!storage_.empty()); storage_.pop_front(); }
    T& front() { assert(!storage_.empty()); return storage_.at(0); }
    T& back() { assert(!storage_.empty()); return storage_.at(storage_.size() - 1); }
...
int main() {
  ...
  cout << "Size after dequeue: " << q.size() << endl; // 2
